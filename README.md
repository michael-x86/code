...
A small 32-bit operating system written entirely in NASM assembly. Boots from BIOS, switches to protected mode with paging, runs the shell as a single task driven by the PIT, and gives you a green-on-black shell with an in-kernel filesystem you can edit and persist to disk.
...
- Single-task shell driven by IRQ0 (100 Hz PIT)
...
- **Kernel** (`kernel.asm`) — sets up paging that identity-maps the first 20 MB **and** maps it at `0xC0000000+`. Then enters the shell task.
