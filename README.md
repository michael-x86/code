...
- Userland binaries linked into `/bin` and loaded on demand: `pwd ls cd cat touch write rm ping vi`
...
- **Userland** — programs are assembled separately into ≤4 KB flat binaries, embedded in `/bin` at build time, loaded to `PROG_LOAD_ADDR` (0xC0700000) when invoked, and talk to the kernel only via `int 0x80`.
...
│   ├── ping.asm  vi.asm (and thenb some...)
...
1. Write `commands/<name>.asm` starting with `[bits 32]` / `[org 0xC0700000]` and ending in `ret`.
...
Programs are capped at 4 KB. Larger scratch buffers should live at fixed virtual addresses past the binary (e.g. `0xC0710000`) — `resb` will zero-fill in flat-binary mode and inflate the file.
...
0xC0700000                PROG_LOAD_ADDR — currently-running userland binary
0xC0710000 – 0xC0712030   vi's scratch buffers (program convention)
