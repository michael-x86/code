#!/usr/bin/env python3
# Boot the OS headless, type the enigma command with args + Enter,
# and screenshot the result.
import socket, subprocess, sys, time, os
SOCK="/tmp/qenigma_cli.sock"
BD=os.path.dirname(os.path.abspath(__file__))
BOOT=os.path.join(BD,"boot.img")
FS=os.path.join(BD,"fs.img")
OUT=os.path.join(BD,"shot_cli.ppm")
...
# Type the command (CLI mode = single line, no Enter until the end)
for ch in CMD:
    k = kn(ch)
