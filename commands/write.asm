...
.get_base:
    ...
    ; Initialize out_ptr to track the start of the text compilation area
...
.next:
    ...
    ; If it's the first word appended (arg 2), skip injecting a separating space
    cmp ebx, 2
    je .skip_sp
...
.done:
    ; Terminate the entire built sentence string with a newline character (\n)
    mov edi, [ebp + out_ptr]
    mov byte [edi], 0x0A    ; '\n'
    inc edi
    
    ; Compute the byte count length of the text compiled inside text_buf
    mov ecx, edi
    lea eax, [ebp + text_buf]
    ...
    cmp eax, -1
    je .err
    ret                     ; Return straight back into exec_bin loader
...
; --- Constant Data Section (Disk Footprint Ends Here) ---
usage_msg db "usage: write <file> <text>", 13, 0
err_msg   db "write: failed", 13, 0

; --- Virtual BSS Section (Allocated dynamically in memory, 0 bytes on disk) ---
    align 4
_bss_start:  equ $
